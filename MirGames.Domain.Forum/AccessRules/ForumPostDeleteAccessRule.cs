// --------------------------------------------------------------------------------------------------------------------
// <copyright company="MirGames" file="ForumPostDeleteAccessRule.cs">
...
namespace MirGames.Domain.Forum.AccessRules
{
    ...
    internal sealed class ForumPostDeleteAccessRule : AccessRule<ForumPost>
    {
        ...
        {
            return principal.IsInRole("User") && principal.GetUserId() == resource.AuthorId && resource.CreatedDate >= DateTime.UtcNow.AddMinutes(-5);
        }
    }
}