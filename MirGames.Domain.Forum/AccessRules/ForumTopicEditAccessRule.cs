// --------------------------------------------------------------------------------------------------------------------
// <copyright company="MirGames" file="ForumTopicEditAccessRule.cs">
...
namespace MirGames.Domain.Forum.AccessRules
{
    ...
    internal sealed class ForumTopicEditAccessRule : AccessRule<ForumTopic>
    {
        ...
        {
            return principal.IsInRole("User") && principal.GetUserId() == resource.AuthorId && resource.CreatedDate >= DateTime.UtcNow.AddDays(-1);
        }
    }
}