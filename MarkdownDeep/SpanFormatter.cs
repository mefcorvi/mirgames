...
namespace Xilium.MarkdownDeep
{
	internal class SpanFormatter : StringScanner
	{
		...
		internal void FormatParagraph(StringBuilder dest, string str, int start, int len)
		{
			...
			// Titled image?
			if (m_Tokens.Count == 1 && m_Markdown.HtmlClassTitledImages != null && m_Tokens[0].type == TokenType.media)
			{
				// Grab the link info
		...
		private void Render(StringBuilder sb, string str)
		{
			foreach (Token t in m_Tokens)
			{
				switch (t.type)
				{
					...
					{
						LinkInfo li = (LinkInfo)t.data;
						li.def.RenderMedia(m_Markdown, sb, li.link_text);
						break;
					}
		...
		{
			// Link or image?
			TokenType token_type = SkipChar('!') ? TokenType.media : TokenType.link;
