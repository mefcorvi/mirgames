...
var MarkdownDeepEditor = new function () {
    ...
    priv.SelectWholeLines = function() {
        ...
        // Move end of selection to end of last line
        if (this.IsStartOfLine(this.m_selectionEnd)) {
            this.m_selectionEnd = this.m_selectionEnd - 1;
        }
        
        this.m_selectionEnd = this.FindEndOfLine(this.m_selectionEnd);
    };
    ...
    */
    ...
    priv.indent_or_outdent = function (state, outdent) {
        ...
        if (state.m_selectionEnd == state.m_selectionStart && selection) {
            state.ReplaceSelection("> " + selection + "\n\n");
            state.m_selectionStart = state.m_selectionEnd + 2;
            state.m_selectionEnd = state.m_selectionStart;
            return true;
        }
        ...
        // Replace
        state.ReplaceSelection(lines.join("\n"));
        state.m_selectionStart = state.m_selectionEnd + 1;
        state.m_selectionEnd = state.m_selectionStart;
    ...
    pub.cmd_link = function(ctx, params) {
        ctx.TrimSelection();
        if (!ctx.CheckSimpleSelection())
            return false;

        var url = params.url || prompt("Введите адрес ссылки:");
        if (url === null)
            return false;

        var text = params.title || ctx.getSelectedText();
        if (text.length == 0) {
            text = "текст ссылки";
        }
        ...
        if (!params.title || !params.url) {
            ...
            ctx.m_selectionStart = ctx.m_selectionEnd;
        }
        
        return true;
    };

    pub.cmd_img = function (ctx, params) {
        ctx.TrimSelection();
        if (!ctx.CheckSimpleSelection())
            return false;

        var url = params.url || prompt("Введите адрес картинки");
        if (url === null)
            return false;

        var alttext = params.title || ctx.getSelectedText();
        if (alttext.length == 0) {
            alttext = "Аннотация";
        }
    ...
    pub.IndentNewLine = function() {
        ...
        var handler = function() {
            ...
            var currentLineStart = state.FindStartOfLine(state.m_selectionStart);
            var currentLineEnd = state.FindEndOfLine(state.m_selectionStart);
            var currentLine = state.m_text.substring(currentLineStart, currentLineEnd);

            // Find start of previous line
            var prevline = state.FindStartOfLine(state.SkipPreceedingEol(state.m_selectionStart));
            var lineEnd = state.FindEndOfLine(prevline);

            var line = state.m_text.substring(prevline, lineEnd);

            // unordered list
            if (line.substr(0, 2) == "* " && currentLine == "") {
                if (line == "* ") {
                    state.m_selectionStart = prevline;
                    state.ReplaceSelection("");
                    state.m_selectionStart = state.m_selectionEnd;
                } else {
                    state.ReplaceSelection("* ");
            ...
            if (match && currentLine == "") {
                if (match[0] == line) {
                    ...
                    state.ReplaceSelection((match[1] * 1 + 1) + ". ");
                    state.m_selectionStart = state.m_selectionEnd + 2;
                }
            }
    ...
    pub.InvokeCommand = function(id, params) {
        params = params || {};
