...
(function (UI) {
    ...
    angular.module('ui.texteditor', ['core.eventBus']).directive('texteditor', function () {
        return {
            ...
            transclude: false,
            template: '<div ng-class="{ \'text-editor-field\': true, \'text-editor-show-preview\': showPreview }">' + '<tinyeditor text="text" post="post()" focus="focus" attachments="attachments" entity-type="{{entityType}}" use-enter-to-post="{{useEnterToPost}}"></tinyeditor>' + '</div>'
        };
    });
})(UI || (UI = {}));
